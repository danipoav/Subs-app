        ...
        <>
            <nav className="fixed top-0 left-0 w-full bg-black bg-opacity-80 backdrop-blur-md z-10 border-b border-gray-700" style={{ color: "#f7f8f8" }}>
                <div className="max-w-6xl mx-auto px-6 flex justify-between items-center h-16">
                    <div onClick={handleBeginning} className="text-xl font-bold text-white flex items-center cursor-pointer">
                    ...
                    <div className="hidden md:flex items-center space-x-2">
                        {status === 'authenticated' ? (
                            ...
                            <>
                                <button onClick={() => navigate("/login")} className="text-gray-300 px-4 py-2 rounded-md hover:bg-gray-900 transition hover:text-white cursor-pointer">Login</button>
                ...
                {/* Mobile Menu */}
                {menuOpen && (
                    <div className="md:hidden bg-black bg-opacity-90 px-6 pb-4 pt-2 space-y-3 text-white text-sm">
                        <a onClick={handleServices} className="block hover:text-gray-300 cursor-pointer">Services</a>
                        {status === 'authenticated' && (
                            <a onClick={handleSubs} className="block hover:text-gray-300 cursor-pointer">Subscriptions</a>
                        )}
                        <a onClick={manageScroll} className="block hover:text-gray-300 cursor-pointer">Management</a>
                        <hr className="border-gray-600" />
                        {status === 'authenticated' ? (
                            <button onClick={handleLogOut} className="block w-full text-left hover:text-gray-300">Logout</button>
                        ) : (
                            <>
                                <button onClick={() => { navigate("/login"); setMenuOpen(false); }} className="block w-full text-left hover:text-gray-300">Login</button>
                                <button onClick={() => { navigate("/register"); setMenuOpen(false); }} className="block w-full text-left hover:text-gray-300">Sign up</button>
                            </>
                        )}
                    </div>
                )}
